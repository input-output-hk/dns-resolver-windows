...
--   Right [93.184.216.34]
--
--   The only error that we can easily cause is a timeout or exhausting the
--   number of retries we set in the resolver. We do this
--   by creating and utilizing a 'ResolvConf' which has a timeout of
--   one millisecond and only 1 retry:
--
--   >>> let hostname = Data.ByteString.Char8.pack "www.example.com"
