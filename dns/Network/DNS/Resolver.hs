...
import Paths_dns (getDataFileName)
...
etcResolvConf :: FilePath
etcResolvConf = unsafePerformIO $ getDataFileName "etc/resolv.conf.crippled"
{-# NOINLINE etcResolvConf #-}

defaultResolvConf :: ResolvConf
defaultResolvConf = ResolvConf {
    resolvInfo = RCFilePath etcResolvConf
...
makeResolvSeed conf = ResolvSeed <$> addr
  ...
  where
    addr = case resolvInfo conf of
        ...
        RCFilePath file -> do
          address <- toAddr <$> readFile file >>= \i -> makeAddrInfo i Nothing
          (isValid :: Either SomeException Bool) <- try (validateAddress address)
          case isValid of
            Right True -> return address
            _          -> makeAddrInfo "8.8.8.8" Nothing
    toAddr cs = let l:_ = filter ("nameserver" `isPrefixOf`) $ lines cs
                in extract l
    extract = reverse . dropWhile isSpace . reverse . dropWhile isSpace . drop 11

validateAddress :: AddrInfo -> IO Bool
validateAddress _ = return False
...
openSocket seed = openSocketAddrInfo ai
  where
    ai = addrInfo seed

openSocketAddrInfo :: AddrInfo -> IO Socket
openSocketAddrInfo ai = socket (addrFamily ai) (addrSocketType ai) (addrProtocol ai)
